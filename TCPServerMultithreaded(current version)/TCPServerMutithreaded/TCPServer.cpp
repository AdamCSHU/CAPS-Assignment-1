...
void TCPServer::receiveData(ReceivedSocketData &ret, bool blocking)
{
	...
	if (blocking || (!blocking && retval > 0))
	{
		...
		if (iResult > 0)
		{
			...
			ret.reply = parseRequests(ret.request);
		}
		//	else if (iResult == 0)
		//	{
		//		recvbuf[iResult] = '\0';
...
std::string TCPServer::parseRequests(std::string request) {
	...
	if (post.valid) {
		return std::to_string(board.postRequest(post.getTopicId(), post.getMessage()));
	}

	ReadRequest read = ReadRequest::parse(request);
	if (read.valid) {
		return board.readRequest(read.getTopicId(), read.getPostId());
	}

	CountRequest count = CountRequest::parse(request);
	if (count.valid)
	{
		return std::to_string(board.countRequest(count.getTopicId()));
	}

	ListRequest list = ListRequest::parse(request);
	if (list.valid)
	{
		return board.listRequest();
	}
	...
	*/
	...
	else if (request.substr(0, 5) == "count" || request.substr(0, 5) == "COUNT") {
		std::string topicID;
		topicID = request.substr(5, 8);
		lock.lock_shared();
		unsigned int messageCount(topicandmessages[topicID].size());
		lock.unlock_shared();
	...
	else if (request.substr(0, 4) == "exit" || request.substr(0, 4) == "EXIT") {
		return "exiting"; 
	}
...
std::vector<std::string> TCPServer::topics()
{
...
inline std::string TCPServer::trun(std::string message, int length)
{
	length = 140;
...
void TCPServer::test(std::string topicID, std::string message) {
	this->topicandmessages[topicID].push_back(message);
}